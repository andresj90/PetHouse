...
Widget buildCardDualPanel(
    var veterinary, BuildContext context, _CareGiverState caregiver) {
  return InkWell(
    onTap: () => {
      caregiver.setState(() => {
            caregiver.about = veterinary["about"],
            caregiver.image = veterinary["image"],
            caregiver.address = veterinary["address"],
            caregiver.name = veterinary["name"]
          })
    },
    child: Card(
      child: ListTile(
...
Widget buildListView(List veterinaryList, BuildContext maincontext, double size,
  ...
  if (size <= 600) {
    ...
    return ListView.builder(
        itemCount: veterinaryList.length,
        itemBuilder: (BuildContext context, int index) {
          return buildCardDualPanel(
              veterinaryList[index], maincontext, caregiver);
        });
  }
}
...
Widget buildDualPanel(List veterinaryList, BuildContext context, double size,
  ...
  return Row(
    children: <Widget>[
      Expanded(
          flex: 2,
          child: buildListView(veterinaryList, context, size, caregiver)),
      Expanded(
        flex: 4,
